...
但这么做也带来几个问题：Sphinx生成出来的HTML和文章源码混合在一起，而git仓库不应该包含任何生成出来的文件，就像不应该把GCC生成的可执行文件放在git仓库里一样；有的浏览器上，按回退无法回到上一个页面刚才看到的那一行，永远会回到页面的最上面；评论功能也不好做，我一开始想 `用Github的issues来做 <https://utteranc.es/>`_ ，看到过其他人的博客有这么实现的，但我不知道该怎么给我这个动态页面做。
